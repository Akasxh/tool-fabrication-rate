...
Tool Fabrication Rate (TFR), that isolates a portable
...
training-free inference-time intervention. Operationally TFR is the
complement of a tool-name validity check, and the ``does the called tool
exist'' question is not itself new --- ToolBeHonest~\citep{toolbehonest2024}
catalogues a \emph{non-existent tool} error category qualitatively. What is
new is not the concept but its per-call operationalization across capability
tiers and vendors, and the content-controlled ablation
(Section~\ref{sec:mechanism}) it makes possible; we report TFR as a
measurement instrument, not a redefinition of hallucination.
...
on denominator and setting. TFR extends MetaTool's premise from
...
metrics cannot expose that contrast. Table~\ref{tab:priorart} positions
TFR against these prior measurements by what each one counts; the
denominators and elicitation setups differ, so the numbers are not
...
A per-call TFR diagnostic disaggregates registry-membership errors
...
tier with $\mathrm{TFR}>0$ (no SFT or RL as in
...
deployments, not a single-model benchmark gain. The TFR scaling
