...
What the design \emph{can} decide is a single binary: does recovery
require the registry \emph{content}, or only the rejection \emph{format}?
If a wrong list ($\mathrm{C}_{0.8}$) recovers as well as the right list
($\mathrm{C}_1$), the content is decorative; if it fails where
$\mathrm{C}_1$ succeeds, the model is reading the offered names. The
