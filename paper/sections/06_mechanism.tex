...
On Qwen3-8B the opaque baseline fabricates $9/615$ calls ($1.46\%$). The naive
retry $\mathrm{C}_{0.5}$ barely moves it ($1/258$, $0.39\%$). The structured error
$\mathrm{C}_{0.7}$ --- a \texttt{tool\_not\_found} envelope with \emph{no registry
attached} --- already drives fabrications to zero ($0/448$, Clopper--Pearson
95\% upper bound $\leq\!0.67\%$), matching full RVR
($\mathrm{C}_1$, $0/258$, $\leq\!1.15\%$). Recovery is bought by the structured rejection, not by
...
$\leq\!0.73\%$) across all four probe arms
(near\_name $0/97$, synonym $0/106$, matched\_random $0/107$, unrelated $0/100$),
indistinguishable from the real list
($\mathrm{C}_1$, $0/258$ on 8B) and from the no-list envelope
($\mathrm{C}_{0.7}$, $0/448$). A wrong list recovers as well as the right one.
...
task-completion rate, not in TFR. We report TFR here because it is the quantity
...
C0 rate is non-monotonic in size --- $0\%$ (0.6B, $0/75$), $0.95\%$ (1.7B),
$1.33\%$ (4B), $1.46\%$ (8B, $9/615$), peaking at $1.87\%$ (14B, $5/268$)
before collapsing to $0\%$ (32B, $0/224$, $\leq\!1.33\%$). Within the probe, the dominant distractor type drifts with scale
